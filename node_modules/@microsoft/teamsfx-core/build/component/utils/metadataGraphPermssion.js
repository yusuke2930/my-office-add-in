...
class MetadataGraphPermissionUtil {
    async parseAadManifest(ymlPath, model, props) {
        ...
        try {
            ...
            if (graphPermissionSummary) {
                ...
                props[telemetry_1.TelemetryProperty.GraphPermissionRoles] = graphPermissionSummary.roles.join(",");
            }
        }
        catch (error) {
            return;
    ...
    summary(manifest) {
        ...
        (_b = graphPermission.resourceAccess) === null || _b === void 0 ? void 0 : _b.forEach((access) => {
            if (access.type === "Role") {
                hasRole = true;
                const id = isUUID_1.default(access.id) ? access.id : graphPermissionMap.roles[access.id];
