...
function QuestionMW(key, fromAction = false) {
    return async (ctx, next) => {
        ...
        const askQuestionRes = await visitor_1.traverse(node, inputs, globalVars_1.TOOLS.ui, globalVars_1.TOOLS.telemetryReporter);
        if (askQuestionRes.isErr()) {
            if (fromAction) {
                ctx.result = {
                    result: teamsfx_api_1.err(askQuestionRes.error),
                    summaries: [],
                };
            }
            else {
