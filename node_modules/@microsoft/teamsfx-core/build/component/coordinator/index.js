...
const generator_4 = require("../generator/officeXMLAddin/generator");
...
const templateNames_1 = require("../generator/templates/templateNames");
const utils_2 = require("../generator/utils");
const M365Actions = [
    "botAadApp/create",
    "teamsApp/create",
    "teamsApp/update",
...
class Coordinator {
    async create(context, inputs, actionContext) {
        ...
        if (scratch === create_1.ScratchOptions.no().id) {
            ...
            const apiMEAuthType = inputs[questionNames_1.QuestionNames.ApiMEAuth];
            ...
            if (projectType === create_1.ProjectTypeOptions.customCopilot().id) {
                lodash_1.merge(actionContext === null || actionContext === void 0 ? void 0 : actionContext.telemetryProps, {
            ...
            if (featureFlags_1.isNewGeneratorEnabled()) {
                ...
                if (capability === create_1.CapabilityOptions.SPFxTab().id) {
                ...
                else if (create_1.ProjectTypeOptions.officeAddinAllIds().includes(projectType)) {
                    const addinHost = inputs[questionNames_1.QuestionNames.OfficeAddinHost];
                    if (projectType === create_1.ProjectTypeOptions.officeXMLAddin().id &&
                        addinHost &&
                        addinHost !== create_1.OfficeAddinHostOptions.outlook().id) {
                        const res = await generator_4.OfficeXMLAddinGenerator.generate(context, inputs, projectPath);
                        if (res.isErr())
                            return teamsfx_api_1.err(res.error);
                    }
                    else {
                    ...
                    const res = await generator_1.CopilotPluginGenerator.generatePluginFromApiSpec(context, inputs, projectPath);
                    if (res.isErr()) {
                        return teamsfx_api_1.err(res.error);
                    }
                    else {
                ...
                else if (meArchitecture === create_1.MeArchitectureOptions.apiSpec().id) {
                    const res = await generator_1.CopilotPluginGenerator.generateMeFromApiSpec(context, inputs, projectPath);
                    if (res.isErr()) {
                        return teamsfx_api_1.err(res.error);
                    }
                    else {
                    ...
                    if (language === "csharp" &&
                        capability === create_1.CapabilityOptions.notificationBot().id &&
                    ...
                    if (inputs.targetFramework &&
                        inputs.targetFramework !== "net6.0" &&
                        inputs.targetFramework !== "net7.0" &&
                        (capability === create_1.CapabilityOptions.nonSsoTab().id ||
                            capability === create_1.CapabilityOptions.tab().id)) {
                        feature = `${capability}:ssr`;
                    }
                    if (capability === create_1.CapabilityOptions.m365SearchMe().id &&
                        meArchitecture === create_1.MeArchitectureOptions.newApi().id) {
                        feature = `${feature}:${apiMEAuthType}`;
                    }
                    if (capability === create_1.CapabilityOptions.customCopilotRag().id) {
                        feature = `${feature}:${inputs[questionNames_1.QuestionNames.CustomCopilotRag]}`;
                    }
                    else if (capability === create_1.CapabilityOptions.customCopilotAssistant().id) {
                        feature = `${feature}:${inputs[questionNames_1.QuestionNames.CustomCopilotAssistant]}`;
                    }
                    const templateName = templateNames_1.Feature2TemplateName[feature];
                    if (templateName) {
                        const langKey = utils_2.convertToLangKey(language);
                        const safeProjectNameFromVS = language === "csharp" ? inputs[questionNames_1.QuestionNames.SafeProjectName] : undefined;
                        const llmService = inputs[questionNames_1.QuestionNames.LLMService];
                        const openAIKey = inputs[questionNames_1.QuestionNames.OpenAIKey];
                        const azureOpenAIKey = inputs[questionNames_1.QuestionNames.AzureOpenAIKey];
                        const azureOpenAIEndpoint = inputs[questionNames_1.QuestionNames.AzureOpenAIEndpoint];
                        const azureOpenAIDeploymentName = inputs[questionNames_1.QuestionNames.AzureOpenAIDeploymentName];
                        context.templateVariables = generator_2.Generator.getDefaultVariables(appName, safeProjectNameFromVS, inputs.targetFramework, inputs.placeProjectFileInSolutionDir === "true", undefined, {
                            llmService,
                            openAIKey,
                            azureOpenAIKey,
                            azureOpenAIEndpoint,
                        ...
                        if (res.isErr())
                            return teamsfx_api_1.err(res.error);
                        if (inputs[questionNames_1.QuestionNames.CustomCopilotRag] === create_1.CustomCopilotRagOptions.customApi().id) {
                            const res = await generator_1.CopilotPluginGenerator.generateForCustomCopilotRagCustomApi(context, inputs, projectPath);
                            if (res.isErr()) {
                                return teamsfx_api_1.err(res.error);
                            }
                            else {
        ...
        if (containsAzure) {
            ...
            if (resourceGroupUnresolved) {
                ...
                if (inputRG && inputLocation) {
                    ...
                    const defaultRg = `rg-${utils_1.convertToAlphanumericOnly(folderName)}${process.env.RESOURCE_SUFFIX}-${inputs.env}`;
    ...
    async deploy(ctx, inputs) {
        ...
        if (projectModel.deploy) {
            ...
            try {
                ...
                ctx.progressBar = (_a = ctx.ui) === null || _a === void 0 ? void 0 : _a.createProgressBar(localizeUtils_1.getLocalizedString("core.progress.deploy"), steps);
