...
exports.ScaffoldRemoteTemplateAction = {
    ...
    run: async (context) => {
        ...
        const zip = await utils_1.fetchZipFromUrl(templateUrl, context.tryLimits, context.timeoutInMs);
        context.outputs = await utils_1.unzip(zip, context.destination, context.fileNameReplaceFn, context.fileDataReplaceFn, context.filterFn);
    },
};
...
async function determineTemplateSource(context) {
    let url = "local";
    if (!Boolean(templates_config_json_1.default.useLocalTemplate)) {
        const latestTag = await utils_1.getTemplateLatestTag(context.language, context.tryLimits, context.timeoutInMs);
        const latestVersion = latestTag.replace(templates_config_json_1.default.tagPrefix, "").trim();
        if (semver_1.default.gt(latestVersion, templates_config_json_1.default.localVersion)) {
            ...
            url = `${templates_config_json_1.default.templateDownloadBaseURL}/${latestTag}/${context.language}.zip`;
        }
    }
    return url;
}
exports.fetchSampleInfoAction = {
    ...
    run: async (context) => {
        const sample = await utils_1.getSampleInfoFromName(context.name);
