...
class DefaultTemplateGenerator {
    ...
    activate(context, inputs) {
        return templateNames_1.tryGetTemplateName(inputs) !== undefined;
    }
    ...
    async run(context, inputs, destinationPath, actionContext) {
        ...
        if (preResult.isErr())
            return teamsfx_api_1.err(preResult.error);
        const templateInfos = preResult.value;
        for (const templateInfo of templateInfos) {
            templateInfo.replaceMap = Object.assign(Object.assign({}, templateReplaceMap_1.getTemplateReplaceMap(inputs)), templateInfo.replaceMap);
    ...
    getTemplateInfos(context, inputs, destinationPath, actionContext) {
        const templateName = templateNames_1.getTemplateName(inputs);
        const language = inputs[question_1.QuestionNames.ProgrammingLanguage];
        return Promise.resolve(teamsfx_api_1.ok([{ templateName, language }]));
    }
    // override this method to do post-step after template download
    post(context, inputs, destinationPath, actionContext) {
        return Promise.resolve(teamsfx_api_1.ok(undefined));
    }
    async scaffolding(context, templateInfo, destinationPath, actionContext) {
        var _a, _b, _c;
        const name = templateInfo.templateName;
        const language = (_a = utils_1.convertToLangKey(templateInfo.language)) !== null && _a !== void 0 ? _a : constant_1.commonTemplateName;
        ...
        const generatorContext = {
            ...
            fileNameReplaceFn: (fileName, fileData) => utils_1.renderTemplateFileName(fileName, fileData, replaceMap)
                .replace(/\\/g, "/")
                .replace(`${name}/`, ""),
            fileDataReplaceFn: (fileName, fileData) => utils_1.renderTemplateFileData(fileName, fileData, replaceMap),
