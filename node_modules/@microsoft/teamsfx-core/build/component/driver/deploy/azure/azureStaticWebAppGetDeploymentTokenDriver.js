...
let AzureStaticWebAppGetDeploymentTokenDriver = AzureStaticWebAppGetDeploymentTokenDriver_1 = class AzureStaticWebAppGetDeploymentTokenDriver {
    constructor() {
        this.description = localizeUtils_1.getLocalizedString("driver.deploy.getSWADeploymentToken");
    }
    async execute(args, ctx, outputEnvVarNames) {
        try {
            ...
            return { result: await common_1.errorHandle(e, ACTION_NAME, ctx.logProvider), summaries: [] };
        }
    }
    static async run(args, ctx, outputEnvVarNames) {
        ...
        const resourceInfo = azureResourceOperation_1.parseAzureResourceId(input.resourceId, AzureStaticWebAppGetDeploymentTokenDriver_1.RESOURCE_PATTERN);
        const azureTokenCredential = await azureResourceOperation_1.getAzureAccountCredential(ctx.azureAccountProvider);
        ...
        const result = teamsfx_api_1.ok(new Map([[outputKey, deploymentKey]]));
        return {
            result: result,
            summaries: [localizeUtils_1.getLocalizedString("driver.deploy.getSWADeploymentTokenSummary")],
        };
    }
};
AzureStaticWebAppGetDeploymentTokenDriver.STORAGE_CONFIG_ARGS = common_1.asFactory({
