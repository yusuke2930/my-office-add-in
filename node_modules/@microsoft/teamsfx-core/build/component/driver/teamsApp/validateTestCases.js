...
let ValidateWithTestCasesDriver = class ValidateWithTestCasesDriver {
    constructor() {
        this.description = localizeUtils_1.getLocalizedString("core.selectValidateMethodQuestion.validate.testCasesOptionDescription");
        this.progressTitle = localizeUtils_1.getLocalizedString("driver.teamsApp.progressBar.validateWithTestCases");
    }
    ...
    async validate(args, context) {
        ...
        if (manifestFile) {
            ...
            lodash_1.merge(context.telemetryProperties, manifestTelemetries);
            ...
            const existingValidationResponse = await appStudioClient_1.AppStudioClient.getAppValidationRequestList(manifest.id, appStudioToken);
                    ...
                    if (validation.status === AsyncAppValidationResponse_1.AsyncAppValidationStatus.InProgress ||
                        ...
                        if (context.platform === teamsfx_api_1.Platform.CLI) {
                            const message = [
                                ...
                                {
                                    content: `${constants_1.getAppStudioEndpoint()}/apps/${manifest.id}/app-validation/${validation.id}`,
                            ...
                            const message = localizeUtils_1.getLocalizedString("driver.teamsApp.progressBar.validateWithTestCases.conflict", `${constants_1.getAppStudioEndpoint()}/apps/${manifest.id}/app-validation/${validation.id}`);
            ...
            const response = await appStudioClient_1.AppStudioClient.submitAppValidationRequest(manifest.id, appStudioToken);
            if (context.platform === teamsfx_api_1.Platform.CLI) {
                const message = [
                    {
                    ...
                    {
                        content: `${constants_1.getAppStudioEndpoint()}/apps/${manifest.id}/app-validation/${response.appValidationId}`,
                        color: teamsfx_api_1.Colors.BRIGHT_CYAN,
                    },
                ];
                ...
                const message = localizeUtils_1.getLocalizedString("driver.teamsApp.progressBar.validateWithTestCases.step", response.status, `${constants_1.getAppStudioEndpoint()}/apps/${manifest.id}/app-validation`);
            ...
            return teamsfx_api_1.err(new common_1.FileNotFoundError(actionName, "manifest.json"));
        }
    }
    /**
    ...
    async runningBackgroundJob(args, context, appStudioToken, response, teamsAppId) {
        const validationRequestListUrl = `${constants_1.getAppStudioEndpoint()}/apps/${teamsAppId}/app-validation`;
        try {
            if (args.showProgressBar && context.ui) {
                ...
                const message = localizeUtils_1.getLocalizedString("driver.teamsApp.progressBar.validateWithTestCases.step", response.status, validationRequestListUrl);
            ...
            while (resultResp.status !== AsyncAppValidationResponse_1.AsyncAppValidationStatus.Completed &&
                ...
                await tools_1.waitSeconds(constants_1.CEHCK_VALIDATION_RESULTS_INTERVAL_SECONDS);
                const message = localizeUtils_1.getLocalizedString("driver.teamsApp.progressBar.validateWithTestCases.step", resultResp.status, validationRequestListUrl);
                context.logProvider.info(message);
                resultResp = await appStudioClient_1.AppStudioClient.getAppValidationById(resultResp.appValidationId, appStudioToken);
            }
    ...
    evaluateValidationResults(args, context, resultResp, teamsAppId) {
        ...
        const validationStatusUrl = `${constants_1.getAppStudioEndpoint()}/apps/${teamsAppId}/app-validation/${resultResp.appValidationId}`;
        ...
        const detailStrArr = [];
        if (failed > 0) {
            summaryStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validate.failed", failed));
            for (const failure of resultResp.validationResults.failures) {
                detailStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases.result.detail", constant_1.SummaryConstant.Failed, failure.title, failure.message));
            }
        }
        if (warns > 0) {
            summaryStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validate.warning", warns));
            for (const warning of resultResp.validationResults.warnings) {
                detailStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases.result.detail", constant_1.SummaryConstant.Warning, warning.title, warning.message));
            }
        }
        if (skipped > 0) {
            summaryStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validate.skipped", skipped));
        }
        if (passed > 0) {
            summaryStrArr.push(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validate.succeed", passed));
        }
        ...
        if (resultResp.status === AsyncAppValidationResponse_1.AsyncAppValidationStatus.Completed) {
            if (args.showMessage && context.ui) {
                void context.ui.showMessage("info", localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases.result", resultResp.status, summaryStr), false);
            }
            context.logProvider.info(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases", resultResp.status, summaryStr, validationStatusUrl, detailStr));
        }
        else {
            if (args.showMessage && context.ui) {
                void context.ui.showMessage("error", localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases.result", resultResp.status, ""), false);
            }
            context.logProvider.error(localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validateWithTestCases", resultResp.status, "", validationStatusUrl, ""));
        }
    }
    validateArgs(args) {
        if (!args || !args.appPackagePath) {
            return teamsfx_api_1.err(new common_1.InvalidActionInputError(actionName, ["appPackagePath"]));
        }
