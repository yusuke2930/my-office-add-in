...
(function (AppStudioClient) {
    ...
    async function importApp(file, appStudioToken, logProvider, overwrite = false) {
        ...
        try {
            ...
            if (((_a = e.response) === null || _a === void 0 ? void 0 : _a.status) === 409) {
                const error = results_1.AppStudioResultFactory.UserError(errors_1.AppStudioError.TeamsAppCreateConflictError.name, errors_1.AppStudioError.TeamsAppCreateConflictError.message(), constants_2.HelpLinks.SwitchAccountOrSub);
                throw error;
            }
    ...
    async function listApps(appStudioToken, logProvider) {
        if (!region)
        ...
        try {
            ...
            const response = await utils_1.RetryHandler.Retry(() => requester.get(`/api/appdefinitions`));
            if (response && response.data) {
                ...
                if (apps) {
                    ...
                    logProvider === null || logProvider === void 0 ? void 0 : logProvider.error("Cannot get the app definitions");
                }
            }
        }
        catch (e) {
            const error = wrapException(e, constants_1.APP_STUDIO_API_NAMES.LIST_APPS);
            throw error;
        }
        throw new Error("Cannot get the app definitions");
    }
    AppStudioClient.listApps = listApps;
    async function deleteApp(teamsAppId, appStudioToken, logProvider) {
        if (!region)
            throw new Error("Failed to get region");
        globalVars_1.setErrorContext({ source: "Teams" });
        let requester;
        try {
            ...
            const response = await utils_1.RetryHandler.Retry(() => requester.delete(`/api/appdefinitions/${teamsAppId}`));
            if (response && response.data) {
            ...
            const error = wrapException(e, constants_1.APP_STUDIO_API_NAMES.DELETE_APP);
            throw error;
        }
        ...
        try {
            ...
            if (response && response.data) {
                ...
                if (app && app.teamsAppId && app.teamsAppId === teamsAppId) {
                    ...
                    // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                    logProvider === null || logProvider === void 0 ? void 0 : logProvider.error(`teamsAppId mismatch. Input: ${teamsAppId}. Got: ${app.teamsAppId}`);
                }
            }
        }
        catch (e) {
                ...
                if (response.data.error) {
                    ...
                    if (response.data.error.code == "Conflict" &&
                        ...
                        try {
                            ...
                            const error = results_1.AppStudioResultFactory.SystemError(errors_1.AppStudioError.TeamsAppPublishConflictError.name, errors_1.AppStudioError.TeamsAppPublishConflictError.message(teamsAppId), e);
                            throw error;
                        }
                    }
    ...
    async function getUserList(teamsAppId, appStudioToken, logProvider) {
        let app;
        try {
    ...
    async function checkPermission(teamsAppId, appStudioToken, userObjectId, logProvider) {
        let userList;
        try {
    ...
    async function grantPermission(teamsAppId, appStudioToken, newUser, logProvider) {
        ...
        try {
            ...
            if (region) {
                try {
                    logProvider.debug(localizeUtils_1.getLocalizedString("core.common.SendingApiRequest", `${baseUrl}/api/appdefinitions/{teamsAppId}/owner`, JSON.stringify(app)));
                    requester = createRequesterWithToken(appStudioToken, region);
                    response = await requester.post(`/api/appdefinitions/${teamsAppId}/owner`, app);
                    logProvider.debug(localizeUtils_1.getLocalizedString("core.common.ReceiveApiResponse", JSON.stringify(response.data)));
                }
                catch (e) {
    ...
    async function getSideloadingStatus(appStudioToken) {
        ...
        const instance = axios_1.default.create({
            baseURL: region !== null && region !== void 0 ? region : constants_1.getAppStudioEndpoint(),
            timeout: 30000,
        });
        instance.defaults.headers.common["Authorization"] = `Bearer ${appStudioToken}`;
        let retry = 0;
    ...
    async function createApiKeyRegistration(appStudioToken, apiKeyRegistration) {
        ...
        try {
            ...
            const error = wrapException(e, constants_1.APP_STUDIO_API_NAMES.CREATE_API_KEY);
            throw error;
        }
    }
    AppStudioClient.createApiKeyRegistration = createApiKeyRegistration;
    /**
    ...
    async function getApiKeyRegistrationById(appStudioToken, apiSecretRegistrationId) {
        ...
        try {
            ...
            const error = wrapException(e, constants_1.APP_STUDIO_API_NAMES.GET_API_KEY);
            throw error;
        }
    }
