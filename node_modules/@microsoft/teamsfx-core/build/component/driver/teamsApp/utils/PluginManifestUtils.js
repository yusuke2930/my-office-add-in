...
class PluginManifestUtils {
    async readPluginManifestFile(path) {
        if (!(await fs_extra_1.default.pathExists(path))) {
            return teamsfx_api_1.err(new common_1.FileNotFoundError("PluginManifestUtils", path));
        }
        ...
        try {
            ...
            return teamsfx_api_1.err(new common_1.JSONSyntaxError(path, e, "PluginManifestUtils"));
        }
    }
    /**
    ...
    async getManifest(path, context) {
        ...
        const resolvedManifestRes = utils_1.getResolvedManifest(JSON.stringify(manifestRes.value), path, telemetry_1.TelemetryPropertyKey.customizedAIPluginKeys, context);
    ...
    async getApiSpecFilePathFromTeamsManifest(manifest, manifestPath) {
        ...
        if (pluginFilePathRes.isErr()) {
            return teamsfx_api_1.err(pluginFilePathRes.error);
        }
    ...
    logValidationErrors(validationRes, platform) {
        ...
        if (platform !== teamsfx_api_1.Platform.CLI) {
            const errors = validationErrors
                ...
                .join(os_1.EOL);
            return (localizeUtils_1.getLocalizedString("driver.teamsApp.summary.validatePluginManifest.checkPath", filePath) +
                os_1.EOL +
                errors);
        }
        else {
            const outputMessage = [];
            outputMessage.push({
                content: localizeUtils_1.getDefaultString("driver.teamsApp.summary.validatePluginManifest.checkPath", filePath) +
                    "\n",
