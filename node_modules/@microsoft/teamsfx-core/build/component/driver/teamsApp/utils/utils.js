...
function normalizePath(path, useForwardSlash) {
    return useForwardSlash ? path.replace(/\\/g, "/") : path;
}
exports.normalizePath = normalizePath;
function getResolvedManifest(content, path, telemetryKey, ctx) {
    const vars = common_1.getEnvironmentVariables(content);
    ctx === null || ctx === void 0 ? void 0 : ctx.addTelemetryProperties({
        [telemetryKey]: vars.join(";"),
    });
    const result = common_1.expandEnvironmentVariable(content);
    const notExpandedVars = common_1.getEnvironmentVariables(result);
    if (notExpandedVars.length > 0) {
        return teamsfx_api_1.err(new error_1.MissingEnvironmentVariablesError("teamsApp", notExpandedVars.join(","), path));
    }
    return teamsfx_api_1.ok(result);
}
exports.getResolvedManifest = getResolvedManifest;
